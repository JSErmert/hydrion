...
class HydraulicsModel:
    """
    Hydraulics model with SAFE config loading.
    Prevents lists/dicts/strings from entering physics.
    """

    def __init__(self, cfg: Any):

        # Safely extract hydraulics section from config
        if hasattr(cfg, "raw") and isinstance(cfg.raw, dict):
            h_raw = cfg.raw.get("hydraulics", {}) or {}
        else:
            h_raw = {}

        # Helper ensures float always returned
        ...
        self.params = HydraulicsParams(
            ...
            P_max_Pa=getf("P_max_Pa", 80000.0),
    ...
    def update(
        ...
        # Decode action
        valve_open = float(np.clip(action[0], 0, 1))
        pump_cmd   = float(np.clip(action[1], 0, 1))
        bf_cmd     = float(np.clip(action[2], 0, 1))
        ...
        n1 = np.clip(Mc1 / max(Mc1_max, 1e-6), 0, 1)
        n2 = np.clip(Mc2 / max(Mc2_max, 1e-6), 0, 1)
        n3 = np.clip(Mc3 / max(Mc3_max, 1e-6), 0, 1)
        ...
        R_valve = p.R_valve_min + (1 - valve_open) * (p.R_valve_max - p.R_valve_min)
        ...
        q_ratio = np.clip(Q_guess / max(p.Q_max_Lmin, 1e-6), 0, 1)
        P_avail = P_max_eff * (1 - q_ratio**2)
        Q_new = P_avail / R_forward * 1000 * 60
        Q_guess = float(np.clip(Q_new, p.min_Q_Lmin, p.Q_max_Lmin))

        diversion = np.clip(p.bf_flow_penalty * bf_cmd, 0, 1)
        Q_out = Q_guess * (1 - diversion)
